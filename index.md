# JS20 STUDIO
## About us
JS20 STUDIO focuses on creating Minecraft resources and Python Programs.

## Our Members
### Founder - Janson20
#### About
Proficient in programming languages such as Python and Java, with 5 years of Python development experience and 1 year of Minecraft mod development experience. Currently responsible for the project 'Jingdu'.

#### Contact
![Banner](https://api.mcbanners.com/banner/saved/YIHretTFTpRdeT.png)
![Banner](https://api.mcbanners.com/banner/saved/eKWlhldyztOexg.png)
- Tencent QQ: `3764579481`
- Email: [janson20@qq.com](mailto:janson20@qq.com)
- Bilibili: [Jansn20](https://space.bilibili.com/661015875)
- Modrinth: [Janson](https://modrinth.com/user/Janson)
- BBSMC: [Janson](https://modrinth.com/user/Janson)
- Nerchat! [Space](https://matrix.to/#/#janson20:chat.neboer.site)
- Torn: [3905052](https://www.torn.com/profiles.php?XID=3905052)
- My Forum: [Account](https://janson20-forum.great-site.net/memberlist.php?mode=viewprofile&u=2)
- Donate at [AiFaDian](https://afdian.com/a/janson20)

#### Recommended Websites
- [My Website](https://janson20.wuaze.com/)
- [My Fourum](https://janson20-forum.great-site.net/)

## Projects
### Jingdu
#### About
A free novel reading & publishing website with no Ads.

#### Links
- View at: https://jingdu.qzz.io or https://jingdu.js20studio.qzz.io/
- Developer: [Janson20](http://github.com/Janson20/)

### DeepSeek For Paper
#### About
DeepSeek for Paper is an intelligent chat and command generation Minecraft plugin that integrates the DeepSeek AI model.
- See more at [Details Page](/DeepSeekForPaper)

#### Download
- Hangar: [DeepSeek For Paper](https://hangar.papermc.io/janson20/DeepSeekForPaper)
- Modrinth: [Chat with DeepSeek](https://modrinth.com/plugin/chatwithdeepseek)
- BBSMC: [DeepSeek for Paper](https://bbsmc.net/plugin/deepseekforpaper)

### Modrinth Mods Downloader
#### About
Modrinth Mods Downloader is an original and easy-to-use tool for downloading Minecraft mods.

#### Download
- BBSMC: [Modrinth模组下载器](https://bbsmc.net/software/modrinth-down)

### Minecraft APK Downloader
#### About
Minecraft APK Downloader is a desktop application developed in Python, specifically designed for browsing and downloading different versions of the Minecraft game APK files. The program retrieves version information and download links by calling the MCAPKS API, offering an intuitive graphical user interface experience.

#### Download
- BBSMC: [Minecraft APK 下载器](https://bbsmc.net/software/mcapks-downloader)

### Spawn Egg Recipes And More
#### About
This mod introduces a unique concept, adding dedicated crafting recipes to certain creatures in the game. These crafting recipes allow players to create spawn eggs for the creatures.

#### Links
- Github: [Spawn-Egg-Recipes-Add-More](https://github.com/Janson20/Spawn-Egg-Recipes-Add-More)

#### Download
- Modrinth: [Spawn Egg Recipes And More(刷怪蛋配方及更多)](https://modrinth.com/mod/spawn-egg-recipes-and-more/)

### Spawn Eggs Trading : Spawn Egg Recipes and more extend
#### About
The 'Spawn Egg Trading' mod is a mod designed specifically for Minecraft, aimed at expanding the trading options with villagers. By installing this mod, players will be able to discover a whole new category of goods—spawn eggs—when trading with villagers. These spawn eggs allow players to purchase them directly from villagers, saving the trouble of searching for materials in the game world and crafting them.

#### Download
- Modrinth: [Spawn Eggs Trading : Spawn Egg Recipes and more extend(刷怪蛋交易)](https://modrinth.com/mod/spawn-eggs-trading)

### Monsters Drop Mine
#### About
The core feature of this data pack is to give certain monsters a chance to drop various minerals, such as iron ingots, gold ingots, diamonds, and so on, when defeated. This not only makes the game more interesting but also provides players with more ways to obtain resources, enriching the gameplay.

#### Download
- Modrinth: [Monsters Drop Mine(怪物掉落矿物)](https://modrinth.com/datapack/monsters-drop-mine)

### Anti-Addiction
#### About
The core function of this resource pack is to guide players to develop healthy gaming habits by reasonably limiting players' game time.

#### Download
- Modrinth: [Anti-Addiction(防沉迷)](https://modrinth.com/resourcepack/anti-addiction)

### Cool
#### About
The "Cool" mod introduces a range of new and convenient features to Minecraft, supporting the 1.21.4neoforge version. By pressing the C key in the game, players can obtain an item named "cool." Right-clicking while holding this item will open a graphical user interface (GUI). Within the GUI, players can click on different buttons to access a variety of practical effects, including instant healing, hunger recovery, jump boost, swiftness, health regeneration, strength, fire resistance, luck, poison, one-click suicide, water breathing, and slowness.

#### Download
- Modrinth: [Cool(酷)](https://modrinth.com/mod/cool-effect)

## Footer
> © 2025 - Now [JS20 Studio](https://js20studio.qzz.io/)
